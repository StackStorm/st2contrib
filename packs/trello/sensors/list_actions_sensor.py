...
class TrelloListSensor(PollingSensor):
    ...
    def __init__(self, sensor_service, config=None, poll_interval=None):
        ...
        list_sensor = self._config.get('list_sensor')
        if not list_sensor:
            raise ValueError('[TrelloListSensor]: "list_sensor" config value is required!')

        self._board_id = list_sensor.get('board_id')
        ...
        self._list_id = list_sensor.get('list_id')
    ...
    def poll(self):
        ...
        _list.fetch_actions(
            action_filter=self._config['list_sensor'].get('filter') or None,
