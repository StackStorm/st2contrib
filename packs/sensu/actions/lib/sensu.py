...
class Sensu():
  ...
  def __init__(self,conf):
    ...
    if self.config['ssl'] == True:
      ...
      protocol = "http"
    self.config['base_url'] = "%s://%s:%s" % (protocol,self.config['host'], self.config['port'])

  def get_headers(self):
    b64auth = base64.b64encode("%s:%s" % (self.config['user'],self.config['pass']))
...
class Aggregates():
  ...
  def list(self,limit=None,offset=None):
    ...
    return parseOutput(requests.get(url=self.url, headers=self.headers,data=data))
...
class Checks():
  ...
  def request(self,check,subscribers):
    ...
    return parseOutput(requests.post(url=url,headers=self.headers,data=payload))
...
class Stashes():
  ...
  def list(self,limit,offset):
    ...
    if offset:
      data['offset'] = offset
    return parseOutut(requests.get(url=self.url,headers=self.headers,data=data))
  ...
  def post(self,data):
    return parseOutput(requests.post(url=self.url,headers=self.headers,data=data))

  def post_by_path(self,path,data):
    url = "%s/%s" % (self.url,path)
    return parseOutput(requests.post(url=url,headers=self.headers,data=data))
...
class Status():
  ...
  def health(self,consumers=2,messages=100):
    ...
    return parseOutput(requests.get(url=url,headers=self.headers,data=data))
